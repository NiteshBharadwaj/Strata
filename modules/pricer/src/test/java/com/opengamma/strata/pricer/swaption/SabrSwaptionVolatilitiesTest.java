...
public class SabrSwaptionVolatilitiesTest {
  ...
  public void test_surfaceCurrencyParameterSensitivity() {
    ...
    for (int i = 0; i < NB_TEST; i++) {
      ...
      SurfaceUnitParameterSensitivity alphaMap = prov.getParameters().getAlphaSurface()
          .zValueParameterSensitivity(expiryTime, TEST_TENOR[i]);
      SurfaceUnitParameterSensitivity betaMap = prov.getParameters().getBetaSurface()
          .zValueParameterSensitivity(expiryTime, TEST_TENOR[i]);
      SurfaceUnitParameterSensitivity rhoMap = prov.getParameters().getRhoSurface()
          .zValueParameterSensitivity(expiryTime, TEST_TENOR[i]);
      SurfaceUnitParameterSensitivity nuMap = prov.getParameters().getNuSurface()
          .zValueParameterSensitivity(expiryTime, TEST_TENOR[i]);
      ...
      assertEquals(alphaMap.getSensitivity().size(), alphaNodeSensiComputed.size());
      assertEquals(betaMap.getSensitivity().size(), betaNodeSensiComputed.size());
      assertEquals(rhoMap.getSensitivity().size(), rhoNodeSensiComputed.size());
      assertEquals(nuMap.getSensitivity().size(), nuNodeSensiComputed.size());
      for (int k = 0; k < alphaNodeSensiComputed.size(); ++k) {
        assertEquals(alphaNodeSensiComputed.get(k), alphaMap.getSensitivity().get(k) * alphaSensi, TOLERANCE_VOL);
      }
      for (int k = 0; k < betaNodeSensiComputed.size(); ++k) {
        assertEquals(betaNodeSensiComputed.get(k), betaMap.getSensitivity().get(k) * betaSensi, TOLERANCE_VOL);
      }
      for (int k = 0; k < rhoNodeSensiComputed.size(); ++k) {
        assertEquals(rhoNodeSensiComputed.get(k), rhoMap.getSensitivity().get(k) * rhoSensi, TOLERANCE_VOL);
      }
      for (int k = 0; k < nuNodeSensiComputed.size(); ++k) {
        assertEquals(nuNodeSensiComputed.get(k), nuMap.getSensitivity().get(k) * nuSensi, TOLERANCE_VOL);
      }
    }
  }
